...
export function SettingsClient({
  ...
  // Preview state — which template is being previewed (shown on right column)
  ...
  const handleTemplateClick = (templateId: string) => {
    // Clicking a card previews it (does NOT select it)
    if (previewId === templateId) return // already previewing
  ...
  return (
    <div className="flex gap-6">
      {/* Left column — forms and template cards */}
      ...
      {/* Right column — inline template preview (sticky) */}
      {role === "doctor" && (
            ...
            {previewId ? (
              <Card className="h-[85vh] flex flex-col">
                ...
                <CardContent className="flex-1 min-h-0 p-0">
                  <div className="h-full flex items-center justify-center bg-muted/40 rounded-b-lg overflow-hidden">
              ...
              <Card className="h-[85vh] flex items-center justify-center">
                <div className="text-sm text-muted-foreground text-center px-8">
                  <FileTextIcon className="size-10 mx-auto mb-3 text-muted-foreground/40" />
                  <p>Click a template to preview it here</p>
                </div>
              </Card>
            )}
          </div>
